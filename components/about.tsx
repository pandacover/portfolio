...
const Span: React.FC<SpanProps> = ({ children, progress, lowerLimit, gradientOne, gradientTwo }) => {
  ...
  return (
    <span className={`transition-all duration-600 ease-linear bg-clip-text text-transparent bg-gradient-to-r ${gradientOne} ${gradientTwo}
    ${currentProgress > lowerLimit && 'drop-shadow-[0rem_0.2rem_0.2rem_#34d399]'}`}>
      {children}
    </span>
  )
}
  ...
  return (
    <div ref={refContainer} className="pt-6">
      <h1 className="transition min-h-fit text-7xl font-bold py-12">
        <Span progress={progress} lowerLimit={-1} gradientOne="from-emerald-400" gradientTwo='to-cyan-600'>Hi Stranger, </Span><br />
        <Span progress={progress} lowerLimit={1} gradientOne="from-emerald-400" gradientTwo='to-cyan-600'>This is</Span><br />
        <Span progress={progress} lowerLimit={3} gradientOne="from-emerald-400" gradientTwo='to-cyan-600'>Luv Makin</Span>
      </h1>
